...
- [x] **Ciclo de vida push ↔ sesión corregido (GUIA_CORRECCION_NOTIFICACIONES §3; 2026-09-24)**: antes el push se habilitaba una vez al arrancar, condicionado a una sesión que se restauraba después (vía guard) → nunca se habilitaba. Ahora `PushSessionCoordinator` observa el usuario del store con un único `effect`: login/restauración/desbloqueo → `enable(userId)`; logout/bloqueada → `disable()`. Idempotente y serializado (sin listeners duplicados, sin re-registro del mismo token); callbacks tardíos de una sesión vieja se descartan por generación (el token de A nunca queda en B). **Probado con mocks (Vitest, 31 tests); NO verificado aún en dispositivo físico.**
- [x] **Baja del token FCM en logout (GUIA_CORRECCION_NOTIFICACIONES §3.4; 2026-09-24)**: endpoint nuevo `DELETE /notifications/inbox/device-token` (backend, autenticado, condicionado a coincidencia exacta del token — no borra un token más nuevo de otro device). El APK lo llama antes de limpiar la sesión; el token queda persistido en storage seguro (`SecureSessionService.saveFcmToken`). Limitación documentada: logout sin red → la asociación remota puede persistir.
- [ ] URL de API en el build del APK: `environment.prod.ts` hoy apunta a `localhost:3001` (config de desktop); el APK necesita la URL pública de `tiendi-api` — **(resuelto en la práctica 2026-08-31: `environment.mobile.ts` apunta a `https://api.tiendi.pe`; el item queda como deuda de doc, verificar antes del próximo release)**
...
- [ ] **Matriz manual en dispositivo** (GUIA_CORRECCION_NOTIFICACIONES §7): login/restore/unlock, logout y cambio de cuenta, permiso concedido/denegado, app foreground/background/cerrada, entrega real de FCM — pendiente de ejecutar con el APK instalado
