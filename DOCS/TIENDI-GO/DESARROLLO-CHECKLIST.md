...
- [ ] Integración real Cloudinary en el paso inicial (step3 del registro) — _el step3 aún almacena URLs que manda el cliente sin pasar por el server; requiere refactorizar `register/step3` para recibir archivos y subirlos al vuelo, similar al endpoint de re-upload ya implementado_
...
- [ ] Contexto de dispositivo (versión de app, OS, GPS) — _el mobile debe enviarlo en `dto.contextBundle`; el server lo preserva pero no lo genera; requiere que el cliente mobile lo capture e incluya al abrir el formulario de soporte_
...
- [ ] FCM normal-priority para: comisión acreditada, mensaje de chat, logro desbloqueado, vínculo de confianza — _comisión acreditada está cubierta por `notifyDeliveryCompleted`; el resto (chat, logros, vínculos de confianza) son módulos de Fase 2 que no existen aún_
