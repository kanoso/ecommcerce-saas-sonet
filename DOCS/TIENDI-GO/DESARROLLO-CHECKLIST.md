...
- [x] **Panel de admin en tiendi-vendor** para revisar y aprobar/rechazar riders — `features/riders/` en tiendi-vendor; SUPER_ADMIN role + guard; `GET /admin/riders` + `GET /admin/riders/:id` en tiendi-api; aprobación/rechazo via `PATCH /admin/riders/:id/status` existente; preview de documentos inline (Cloudinary)
- [x] **Integración real Cloudinary en `register/step3`** — corregido: backend ya usaba `FileFieldsInterceptor` + `CloudinaryService`; mobile tenía rutas `/auth/register/*` → fijas a `/riders/register/*` y field names en español → inglés
...
- [x] Panel de admin en tiendi-vendor para revisar y aprobar/rechazar riders — implementado en `tiendi-vendor/features/riders/` + 2 endpoints nuevos en tiendi-api (`GET /admin/riders`, `GET /admin/riders/:id`)
...
- [x] `POST /auth/logout-all` — invalida el access token actual (blacklist Redis); best-effort — refresh tokens son JWT stateless sin store
- [x] `POST /riders/me/data-export` — solicita exportación de datos (202 + email futuro; MVP = log de intención)
- [x] `DELETE /riders/me` — soft-delete + anonimización GDPR: status INACTIVE, PII a null/sentinel, User.email a deleted-{id}@anon.tiendi; login bloqueado via JwtStrategy
...
#### Mobile
- [x] Pantalla de zonas de cobertura con `MapView` interactivo (dibujar radio o polígono) — `settings-coverage-zones.tsx` (un radio, pill-row 5/8/10/12/15 km, MapView + Circle, GPS default center; `PATCH /riders/me/coverage-zones`; MVP: una sola zona activa)
- [ ] Selector de zona activa (si tiene más de una) — _deferred Fase 2 (MVP soporta 1 zona)_
- [x] Preferencias de pedidos: método de pago, radio, multi-pedido — `settings-preferences.tsx` + mapper `mapPreferencesToApi()` en `ridersService` (convierte `acceptCashOrders/acceptDigitalOrders/maxRadiusKm/acceptMultiOrder` → `paymentMethod/acceptanceRadiusKm/multiOrder` antes de enviar a la API)
...
- [x] Tests unitarios de stores de Zustand (cobertura ≥ 90%) — 128 tests nuevos (location/delivery/auth/wallet/support); 97% stmts, 94% branches; también corregido bug `storage.remove` → `storage.delete` en delivery.store
- [x] Tests de schemas Zod (cobertura 100%) — 26 tests en auth.schemas; también gps-queue.store (19) y notification-inbox.store (25)
