...
- [ ] Modelo laboral definido: ¿riders propios (empleados) o asociados (marketplace)?
- [ ] Política de efectivo definida: ¿quién asume el riesgo si el cliente no paga?
- [ ] Algoritmo de asignación definido: ¿más cercano, mejor rating, round-robin, subasta?
- [ ] Tarifas base y comisión de la plataforma configuradas por zona
- [ ] SLA de revisión de documentos definido (48hs hábiles por defecto)
...
- [ ] Inicializar proyecto con `npx create-expo-app --template` + TypeScript strict
- [ ] Configurar Expo Router con estructura de carpetas `(auth)` / `(app)`
- [ ] Configurar `app.json`: bundle ID iOS (`com.tiendi.go`), package Android (`com.tiendi.go`), permisos GPS y cámara
- [ ] Configurar `eas.json` con perfiles `development`, `preview` y `production`
- [ ] Instalar y configurar dependencias base: `zustand`, `axios`, `socket.io-client`, `react-native-maps`, `expo-location`, `expo-camera`, `expo-notifications`, `expo-secure-store`, `expo-local-authentication`, `react-hook-form`, `zod`, `date-fns`, `react-native-mmkv`, `react-native-reanimated`, `react-native-gesture-handler`
- [ ] Crear stores de Zustand: `useAuthStore`, `useDeliveryStore`, `useLocationStore`, `useWalletStore`
- [ ] Configurar instancia de axios con interceptors para JWT y refresh token
- [ ] Configurar cliente Socket.IO con reconexión automática
- [ ] Google Maps SDK configurado con API key en `app.json` (Android + iOS)
...
#### Backend
- [ ] `GET /riders/me` — perfil completo del rider
- [ ] `PATCH /riders/me` — edición de campos libres (foto, zona)
- [ ] `POST /riders/me/update-request` — campos con re-validación (nombre, teléfono, email)
- [ ] `GET /riders/me/vehicles` — lista de vehículos
- [ ] `POST /riders/me/vehicle-change-request` — solicitud de cambio de vehículo con nuevos documentos
- [ ] `PATCH /riders/me/status` — cambiar estado (`Online` / `Offline` / `EnPausa`)
- [ ] Cron: pausa que expira después de 4hs → pasa automáticamente a `Offline`
- [ ] Push 15 min antes del vencimiento de la pausa
- [ ] Cron: rider sin actividad 30 días → estado `Inactivo`

#### Mobile
- [ ] Pantalla de perfil con secciones: foto, datos personales, vehículo, documentos, cuenta bancaria, métricas
- [ ] Toggle Online / Offline en pantalla principal con confirmación
- [ ] Estado "En Pausa" con countdown visible y botón de reanudar
- [ ] Edición de foto de perfil
- [ ] Campos con re-validación: badge "Pendiente de aprobación" durante revisión
- [ ] Solicitud de cambio de vehículo con upload de documentos
- [ ] Métricas visibles en perfil: rating, pedidos del mes, nivel de puntuación
