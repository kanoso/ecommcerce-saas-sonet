...
- [x] **F7.5 — Lecturas derivadas** (resuelto 2026-08-27): `getTransactions` sirve desde `LedgerEntry` filtrado por las cuentas del rider (`ownerType: RIDER`), con mapeo de kinds a tipos legados (`DELIVERY_COMMISSION→COMMISSION`, `RIDER_WITHDRAWAL→WITHDRAWAL`, `CASH_DEPOSIT_CONFIRMED→CASH_DEPOSIT`) y signo invertido para vista de rider; API y contrato intactos. La tabla `Transaction` queda deprecated (marcada en schema), no eliminada. Nota: los eventos de neteo interno (`COMMISSION_RELEASE`, W4) no aparecen en el historial porque no son hechos económicos, solo reestructuras de proyección.
- [~] **F7.6 — Suite verde + verificación doble-escritura**: suite 529/529; la doble escritura quedó activa por diseño (proyección + asiento atómicos) e I8 la audita diariamente. Falta el período de sombra en un entorno con datos antes de deprecar la escritura directa de proyecciones.
