...
4. Confirmar qué runtimes están desplegados realmente — **VERIFICADO (2026-09-26, dos pasadas SSH)**: PM2 con `tiendi-platform-api` (API plataforma, 3001), `tiendi-api` (**API de kipu** — nombre invertido, riesgo de confusión operativa), `tiendi-web` (**SSR**: `dist/tiendi-web/server/server.mjs`, 4000, health 200), `tiendi-shield` (**shield-server**: `server.mjs`, 4203, health 200), y estáticos `pm2 serve` (vendor/admin/kipu/site/valia desde `C:\tiendi\...`); Docker con postgres/redis/prometheus/loki/grafana:latest (3002)/openbao-test:2.7.0. Consecuencia OTel: SSR usa `TIENDI_SERVICE_NAME=tiendi-web-ssr` para distinguirse del browser; shield-server desplegado → decisión D6 aplica.
